...
function skyboxColor(y){
  // Reference v5: large golden-orange base -> soft cream band -> pale blue-grey cap (top).
  // Mapping uses existing sky bounds; everything else remains unchanged.
  ...
  // Preserve a broad warm area; compress upper bands slightly
  t = Math.pow(t, 0.85);

  // Palette approximated from the provided image
  const orange = [0.96, 0.62, 0.05]; // ~#F59E0D golden orange
  const cream  = [0.95, 0.85, 0.64]; // ~#F2D9A4 soft cream
  const blue   = [0.85, 0.89, 0.94]; // ~#D8E3F0 pale blue-grey

  // Split points bottom(0)->top(1)
  const s1 = 0.78; // orange -> cream (very broad base)
  const s2 = 0.92; // cream  -> blue  (narrow top cap)
