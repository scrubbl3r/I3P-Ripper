...
function skyboxColor(y){
  // Reference v6: broad orange base -> thin cream band -> pale blue-grey cap.
  // Uses existing sky bounds for mapping; leaves the wave + under-wave grades untouched.
  ...
  // Ease to keep the warm base broad and the transition bands tight
  t = Math.pow(t, 0.88);

  // Palette approximated from your latest image
  const orange = [0.96, 0.64, 0.08]; // ~#F5A314 rich orange
  const cream  = [0.95, 0.86, 0.69]; // ~#F2DBB0 thin cream
  const blue   = [0.84, 0.87, 0.91]; // ~#D5DEE8 pale blue-grey

  // Split points bottom(0)->top(1)
  const s1 = 0.74; // orange -> cream (very broad base)
  const s2 = 0.86; // cream  -> blue  (short cream band, tall blue cap)

  function mix(a,b,u){ return a + (b-a)*u; }
  function mix3(A,B,u){ return [mix(A[0],B[0],u), mix(A[1],B[1],u), mix(A[2],B[2],u)]; }

  if (t <= s1){
    return mix3(orange, cream, t / s1);
  } else if (t <= s2){
    return mix3(cream, blue, (t - s1) / (s2 - s1));
  } else {
    return blue;
  }
}
