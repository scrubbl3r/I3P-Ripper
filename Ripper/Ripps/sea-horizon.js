...
function skyboxColor(y){
  let t = clamp((y - ctx.skyYMin) / (ctx.skyYMax - ctx.skyYMin), 0, 1);
  t = cubicEase(t, 0.85); // hold the orange longer
  const warm   = [1.00, 0.50, 0.12];
  const midC   = [0.90, 0.88, 0.98];
  const zenith = [0.36, 0.56, 1.00];
  const split = 0.60; // extend warm higher
  const a = clamp(t / split, 0, 1);
  const b = clamp((t - split) / (1 - split), 0, 1);
  const warmToMid = mix3(warm,  midC,  a);
  return             mix3(warmToMid, zenith, b);
}
