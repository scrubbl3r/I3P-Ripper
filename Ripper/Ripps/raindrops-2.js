...
const LIFETIME_MS      = 2000;     // travel time top→bottom (per drop)
const SPAWN_MIN_MS     = 25;       // random spawn window
...
function progressDown(u){
  ...
  const slow = Math.pow(u, 3.0);           // strong ease-in (very slow start)
  const mid = 0.5, width = 0.22, boost = 0.35; // tune: wider/narrower & amount of extra speed
  const t = Math.max(0, 1 - Math.abs((u - mid)/width));
  const bell = t*t*(3 - 2*t);               // smoothstep bell in [0,1]
  let s = slow + boost * bell * u;          // add equator push; scaled by u to keep start slow
...
export function init(api){
  ...
  drops.push({ bornMs: 0, az: Math.random() * Math.PI * 2, r: DROP_RADIUS_WU * (1 + randIn(-0.2, 0.2)) });
  paint(api, 0);
}
...
export function update(api, t/*s*/, dt/*s*/){
  ...
  while (emitAccMs >= nextDelayMs){
    ...
    drops.push({ bornMs: nowMs, az: Math.random() * Math.PI * 2, r: DROP_RADIUS_WU * (1 + randIn(-0.2, 0.2)) });
    nextDelayMs = randIn(SPAWN_MIN_MS, SPAWN_MAX_MS);
  }
