...
function skyboxColor(y){
  // New reference gradient v3:
  // deep red-brown (bottom) -> orange -> golden yellow -> light cream -> soft blue (top cap)
  // Uses existing sky bounds for mapping; animation/under-wave grades unchanged.
  ...
  // Ease to keep a broad lower warm area and short blue cap
  t = Math.pow(t, 0.8);

  // Palette sampled from your image (approx SRGB -> linear assumed by shader)
  const brown   = [0.62, 0.28, 0.19]; // ~#9F4731
  const orange  = [0.88, 0.41, 0.24]; // ~#E1693D
  const gold    = [0.95, 0.65, 0.10]; // ~#F2A61A
  const cream   = [0.96, 0.88, 0.72]; // ~#F6E0B8
  const blueTop = [0.49, 0.63, 0.84]; // ~#7EA0D6

  // Split points (bottom -> top), tuned to the provided reference:
  const s1 = 0.58; // brown -> orange (sharp-ish shoulder)
  const s2 = 0.66; // orange -> gold (thin vivid band)
  const s3 = 0.82; // gold   -> cream (gentle blend)
  const s4 = 0.92; // cream  -> blue  (narrow top cap)

  function mix(a,b,u){ return a + (b-a)*u; }
  function mix3(A,B,u){ return [mix(A[0],B[0],u), mix(A[1],B[1],u), mix(A[2],B[2],u)]; }

  if (t <= s1){
    // long brown base
    return mix3(brown, orange, t / s1);
  } else if (t <= s2){
    // punchy orange band
    return mix3(orange, gold, (t - s1) / (s2 - s1));
  } else if (t <= s3){
    // golden into cream
    return mix3(gold, cream, (t - s2) / (s3 - s2));
  } else if (t <= s4){
    // cream into blue
    return mix3(cream, blueTop, (t - s3) / (s4 - s3));
  } else {
    // soft blue cap
    return blueTop;
  }
